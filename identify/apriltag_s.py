...
from identify import tagFamilies as tf
import numpy as np
import time
...
class Apriltag(object):
    ...
    def create_detector(self,family = 'rmtag',sigma=0.8,nthread =1,debug = False,minarea = 400,thresholding = 'adaptive',downsampling = False):
        '''
        ...
        if(family == 'tag36h11'):
            ...
            self.tagfamily = tf.RMTAGclass(debug = self._debug)
        # else:
    ...
    def detect(self,frame):
        ...
        # if (self._debug):
        #     plt.figure().set_size_inches(9.2, 5.8)
        #     plt.imshow(img)
        #     plt.gray()
            # plt.show()
...
#            img = cv2.GaussianBlur(img, (7, 7), self._quad_sigma)
        ...
        # if self._debug
            ...
            # cv2.drawContours(framecopy, contours, -1, (0, 0, 255), 2)
        ...
        if self._debug:
            ...
            framecopy = np.copy(frame)
            # cv2.drawContours(frame, quads, -1, (0, 0, 255), 2)
        ...
        """
        a,b = self.tagfamily.decodeQuad(quads,gray)
        return a,b,quads